...
package new_test_project.hereissomething_0_1;
...
public class HereIsSomething implements TalendJob {
	...
	private final String projectName = "NEW_TEST_PROJECT";
	...
	public static class row1Struct implements
		...
		final static byte[] commonByteArrayLock_NEW_TEST_PROJECT_HereIsSomething = new byte[0];
		static byte[] commonByteArray_NEW_TEST_PROJECT_HereIsSomething = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_NEW_TEST_PROJECT_HereIsSomething.length) {
					if (length < 1024
							&& commonByteArray_NEW_TEST_PROJECT_HereIsSomething.length == 0) {
						commonByteArray_NEW_TEST_PROJECT_HereIsSomething = new byte[1024];
					} else {
						commonByteArray_NEW_TEST_PROJECT_HereIsSomething = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_NEW_TEST_PROJECT_HereIsSomething,
						0, length);
				strReturn = new String(
						commonByteArray_NEW_TEST_PROJECT_HereIsSomething, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_NEW_TEST_PROJECT_HereIsSomething) {
		...
		try {
			...
			java.io.InputStream inContext = HereIsSomething.class
					.getClassLoader().getResourceAsStream(
							"new_test_project/hereissomething_0_1/contexts/"
